...
import type { EvaluationResult, Obligation } from '@/src/lib/types';

export default function ViewResultsPage() {
  const [payload, setPayload] = useState<{ evaluation: EvaluationResult; obligations: Obligation[] } | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    try {
      const hash = typeof window !== 'undefined' ? window.location.hash.slice(1) : '';
      if (!hash) { setError('No results provided.'); return; }
      const json = decodeURIComponent(atob(hash));
      const parsed = JSON.parse(json);
      setPayload({ evaluation: parsed.evaluation, obligations: parsed.obligations });
    } catch {
      setError('Invalid results payload.');
    }
  }, []);

  const groups = useMemo(() => {
    if (!payload) return {} as Record<string, { obligation: Obligation; eval: EvaluationResult['appliedObligations'][number] }[]>;
    const byId = new Map(payload.obligations.map((o) => [o.id, o] as const));
    const applied = payload.evaluation.appliedObligations.filter((a) => a.applied);
    const g: Record<string, { obligation: Obligation; eval: typeof applied[number] }[]> = {};
    for (const e of applied) {
      ...
      g[key] = g[key] || [];
      g[key].push({ obligation: ob, eval: e });
    }
    return g;
  }, [payload]);
