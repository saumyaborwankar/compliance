...
export default function LocalResultsPage() {
  const [data, setData] = useState<{ evaluation: EvaluationResult; obligations: Obligation[] } | null>(null);

  useEffect(() => {
    const raw = sessionStorage.getItem('latest_evaluation');
    if (raw) {
      try {
        const parsed = JSON.parse(raw);
        setData({ evaluation: parsed.evaluation, obligations: parsed.obligations });
      } catch {
        setData(null);
      }
    }
  ...
  if (!data) return <div className="p-6">No local evaluation found. Please submit the intake again.</div>;

  const { evaluation, obligations } = data;
  const byId = new Map(obligations.map((o) => [o.id, o] as const));
  const applied = evaluation.appliedObligations.filter((a) => a.applied);
